...
const CalcRow = ({ id, activeNalogusn, defStore = "", defCategory = "", defSubject = "",
    ...
    return isRemoved ? "" :
        <tr>
            ...
            <td>{parseFloat(((price - (subjectData.CostStorageBox * turnover) - logisticMarketplace - (commission * price / 100) - logistic - packaging - cost) / 100 * 15).toFixed(2))}&nbsp;&nbsp;₽</td>
            <td>{parseFloat((((price - (subjectData.CostStorageBox * turnover) - logisticMarketplace - (commission * price / 100) - logistic - packaging - cost) / 100 * 15) * count).toFixed(2))}&nbsp;&nbsp;₽</td>
        </tr>
}



export const Nalogusn = () => {
    const [activeNalogusn, setActiveNalogusn] = useState(0);
    const auth = useAuth();
    const subjectsDataFunc = useSubjectsData();
    ...
    useEffect(() => {
        ...
        subjectsDataFunc.getCalculations(auth.user.id).then(res => {
            ...
            const newSections = res.map(data => {
                // console.log(data);
                return <CalcRow auth={auth} subjectsDataFunc={subjectsDataFunc} id={data.id} activeNalogusn={activeNalogusn}
    ...
    return (
        ...
        <section className="home">
            ...
            <div className="block">
                ...
                <div className="block_nalog15">
                    <div className="rectangle"></div>
                </div>

                <div className="textnalog15">Упрощенная система налогооблажения</div>
                <div className="textnalog15two">Доходы-Расходы 15%</div>

                <div className="banerusn15">
                    <a href="https://www.marketstat.app/nalog15"><img src="/img/Banerusn15.svg" height="150px" /></a>
                </div>
